...
//Geliştirdiğimiz uygulamalarda ister istemez verisel olarak tutarsızlıklar meydana gelebilmektedir. Örneğin; birden fazla uygulamanın yahut client'ın aynı veritabanı üzerinde eşzamanlı olarak çalıştığı durumlarda verisel anlamda uyuglamadan uygulamaya yahut client'tan clienta tutarsızlıklar meydana gelebilir.
...
// Şöyle bir örnek verebiliriz 100 tl fiyata sahip bir kıyafet düşünün o müşteri o kıyafet üzerinde işlem yaparken bir anda fiyatı 150 tl olarak güncellendi bu bir veri tutarsızlığına yol açabilir. Ya işlem yaptırmayacağız yada farklı yollar kullanıcaz. GORECEGIZ
#endregion
...
//Kitlenmiş olan bir verinin veirtabanı seviyesinde meydana gelen sistemsel bir hatadan dolayı kilidinin çözülememesi yahut döngüsel olarak kilitlenme durumunun meydana gelmesini ifade eden bir terimdir.(Veritabanını resetlemediğin sürece devam etmesi muhtemel bir durumdur)
...
#endregion
#region WITH (XLOCK)
// Bu Yapı Pessimistic Lock düşüncesinden üretilmiştir. Ancak bu yol pek kullanılmaması gereken bir yöntemdir. DeadLock'a sebeb olur ve DB kitlenir. Çözmenin tek yoluda DB'ye reset atmak olur. Kötü bir yol.
// Kullanımı için aşağıda ki örnek te EFCORE' da şöyle birşey yapabiliyoruz. Transaction'u başlatıp o arada Veriyi kitliyoruz ta ki transaction Commit edilene kadar.
...
//Optimistic lock yönteminde, Pessimistic lock'da olduğu gibi veriler üzerinde tutarsızlığa mahal olabilecek değişiklikler fiziksel olarak engellenmemektedir. Yani veriler tutarsızlığı sağlayacak şekilde değiştirilebilir. 
...
class ApplicationDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        //modelBuilder.Entity<Person>().Property(p => p.Name).IsConcurrencyToken(); [ConcurrencyCheck] yapısının fluent api hali.
